# Documentation

Project documentation is grouped by topic so planning notes, implementation guides, and operational references stay easy to find.

## Folders

- [analysis](analysis/) - Analysis engine test data and fixture guidance.
- [api](api/) - Public API contracts and response references.
- [auth](auth/) - Authentication, signup/login, payment, enrollment, and access-control plans.
- [design](design/) - UI and visual design conventions.
- [engineering](engineering/) - Environment, deployment, scaling, and troubleshooting notes.
- [product](product/) - Product vision, feedback specs, bug lists, and feature priorities.

## Quick Links

- [Environment Variables](engineering/env-vars.md)
- [Troubleshooting](engineering/troubleshooting.md)
- [Public Candidate Analysis API](api/public-candidate-analysis-api.md)
- [Auth Integration Guide](auth/auth-integration-guide.md)
- [Payment & Access Flow](auth/payment-integration-plan.md)
- [Feature Priority List](product/feature-priority-list.md)
- [Bug Priority List](product/bug-priority-list.md)
